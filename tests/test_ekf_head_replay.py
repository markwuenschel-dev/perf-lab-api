...
async def _set_soreness(factory, sample_id: int, soreness: float) -> None:
...
async def test_eligible_head_correction_replays_immediately(factory):
    ...
    # original reconciled; numerical row untouched; flag cleared.
...
async def test_reconcile_cannot_clear_a_newer_pending_generation(factory):
    ...
    async with factory() as db:
        await ekf_shadow_service._reconcile_after_replay(db, original_b.id, claimed_revision=1, replay_id=original_b.id)
        await db.commit()

    refreshed = _one(await _rows(factory, uid), event_type="update", source_id=b)
    assert refreshed.replayed_revision == 1              # advanced to the claimed generation
    assert refreshed.correction_requires_replay is True  # rev 2 still pending -> NOT cleared
