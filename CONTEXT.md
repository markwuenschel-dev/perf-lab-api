...
One persisted `AthleteState` row surfaced by `/state-history` — a complete `UnifiedStateVector` at one `timestamp`. The unit of the Digital Twin's time-travel axis. Snapshots are written on state **updates** (a workout processed, a check-in), so they are **event-cadenced, never daily**: adjacent snapshots may be minutes or weeks apart, and **snapshot-index distance is not elapsed time**. Any calendar label ("Today", "Yesterday", an absolute date) is derived from the snapshot's own `timestamp`, never from its position in the list. A single-row history is a *thin* history (one real snapshot), distinct from an *empty* one (no rows) — and neither is a licence to manufacture default `50` values as if measured.
_Avoid_: "day", "daily readiness", treating a slider index as a time offset.

## Window start / oldest loaded state
The oldest snapshot in the **current** `/state-history` response — the comparison point for "change across loaded history" (capacity deltas, radar vs-start). Because the endpoint is bounded (default 60, max 365 rows), it is only the oldest *loaded* row, and it **moves** as history accumulates. It is **not** a durable physiological baseline or block start, and must not be labelled "baseline"/"block start" unless the backend exposes an actual baseline/block identity. Insufficient-confidence axes are excluded from window-start comparison entirely (see [Evidence status vs confidence status](#evidence-status-vs-confidence-status)).
_Avoid_: "baseline", "block start" (for this moving comparison point).
