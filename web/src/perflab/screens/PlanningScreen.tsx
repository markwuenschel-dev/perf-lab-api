...
export function PlanningScreen() {
  ...
  const { data: sessions, loading } = useAuthedResource<PlannedSessionRead[]>(
    (t) => api.listPlannedSessions(t, { start_date: week.start_date, end_date: week.end_date }),
    [week.start_date, state.planningRefreshKey],
  );

  // A signed-in athlete with no planned sessions has no active block — this is
  // the dead end the prototype WEEK used to paper over. Show a CTA instead.
  const hasNoBlock = !!auth.token && !loading && (!sessions || sessions.length === 0);
  if (hasNoBlock) {
    return <PlanningEmptyState onCreate={actions.openBlockCreate} />;
  }
